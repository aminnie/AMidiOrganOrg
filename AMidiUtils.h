...
struct MidiSysExRouteEntry final
{
    juce::String inputIdentifier;
    juce::String outputIdentifier;
    juce::String outputModule;
};

/** Explicit SysEx input-to-output routes (`configs/midi_sysex_routes.json`). */
inline juce::File getMidiSysExRoutesFile()
{
    juce::File dir = getOrganUserDocumentsRoot().getChildFile(getAppState().configdir);
    if (!dir.isDirectory())
        dir.createDirectory();
    return dir.getChildFile("midi_sysex_routes.json");
}

/** Reads explicit SysEx routes; missing/invalid file is treated as empty routes. */
inline void loadMidiSysExRoutesFromFile(juce::Array<MidiSysExRouteEntry>& routes)
{
    routes.clear();

    const juce::File f = getMidiSysExRoutesFile();
    if (!f.existsAsFile())
        return;
    ...
    const juce::var routesVar = obj->getProperty("routes");
    if (!routesVar.isArray())
        return;

    const auto* arr = routesVar.getArray();
    if (arr == nullptr)
        return;

    for (const auto& routeVar : *arr)
    {
        ...
        route.inputIdentifier = routeObj->getProperty("inputIdentifier").toString().trim();
        route.outputIdentifier = routeObj->getProperty("outputIdentifier").toString().trim();
        route.outputModule = routeObj->getProperty("outputModule").toString().trim();

        if (route.inputIdentifier.isEmpty())
            continue;

        if (route.outputIdentifier.isEmpty() && route.outputModule.isEmpty())
            continue;

        bool duplicateInput = false;
        for (const auto& existing : routes)
        {
            if (existing.inputIdentifier.equalsIgnoreCase(route.inputIdentifier))
            {
                duplicateInput = true;
                break;
            }
...
/** Overwrites explicit SysEx route file; returns false on directory/create/write failure. */
inline bool saveMidiSysExRoutesToFile(const juce::Array<MidiSysExRouteEntry>& routes)
{
    const juce::File f = getMidiSysExRoutesFile();
    if (!f.getParentDirectory().createDirectory())
        return false;
    ...
    for (const auto& route : routes)
    {
        ...
        const juce::String outputIdentifier = route.outputIdentifier.trim();
        const juce::String outputModule = route.outputModule.trim();

        if (inputIdentifier.isEmpty())
            continue;
        if (outputIdentifier.isEmpty() && outputModule.isEmpty())
            continue;

        juce::DynamicObject::Ptr routeObj = new juce::DynamicObject();
        routeObj->setProperty("inputIdentifier", inputIdentifier);
        if (outputIdentifier.isNotEmpty())
            routeObj->setProperty("outputIdentifier", outputIdentifier);
        if (outputModule.isNotEmpty())
            routeObj->setProperty("outputModule", outputModule);
        routesArray.add(juce::var(routeObj.get()));
    }

    juce::DynamicObject::Ptr root = new juce::DynamicObject();
    root->setProperty("version", 1);
    root->setProperty("routes", juce::var(routesArray));

    const juce::String txt = juce::JSON::toString(juce::var(root.get()), true);
    return f.replaceWithText(txt);
}
